...
# Neolink 刊例价接口要登录；默认 0，只记 usage。确认单价后设 NL_CNY_PER_M_IN/OUT。
NL_IN = float(os.environ.get("NL_CNY_PER_M_IN", "0"))
NL_OUT = float(os.environ.get("NL_CNY_PER_M_OUT", "0"))
NL_BUDGET = float(os.environ.get("NL_BUDGET_CNY", "200"))
...
def one_call(
    *,
    ...
    if target == "local":
        ...
        url = NL_BASE + "/chat/completions"
        headers = {"Authorization": f"Bearer {NL_KEY}"}
        model = NL_MODEL
    else:
    ...
    bill = None
    if target == "sf":
        bill = sf_bill(float(pt or 0), float(ct or 0), cached)
    elif target == "neolink" and (NL_IN or NL_OUT):
        miss = max(float(pt or 0) - cached, 0.0)
        bill = {
            "total_cny": miss / 1e6 * NL_IN + cached / 1e6 * NL_IN + float(ct or 0) / 1e6 * NL_OUT
        }
    ...
    return {
        ...
        "bill_cny": None if bill is None else round(bill["total_cny"], 6),
        "error": err,
    }
...
def run_level(
    *,
    ...
    if target == "neolink":
        if not accept_nl:
            raise SystemExit("neolink run refused: pass --i-accept-nl-cost")
        if not NL_KEY:
            raise SystemExit("neolink run refused: NEOLINK_API_KEY is empty")
    ...
    lock = threading.Lock()
    stop_at = 0.0
    budget = Budget(SF_BUDGET) if target == "sf" else (Budget(NL_BUDGET) if target == "neolink" and (NL_IN or NL_OUT) else None)
    submitted = 0
    in_flight = 0
...
def cmd_run(args: argparse.Namespace) -> None:
    ...
    if target == "sf" and not SF_KEY:
        raise SystemExit("sf run refused: SILICONFLOW_API_KEY is empty")
    if target == "neolink" and not getattr(args, "i_accept_nl_cost", False):
        raise SystemExit("refused: neolink requires --i-accept-nl-cost")
    if target == "neolink" and not NL_KEY:
        raise SystemExit("neolink run refused: NEOLINK_API_KEY is empty")
    ...
    for i, c in enumerate(concs):
        rec = run_level(
            ...
            accept_nl=getattr(args, "i_accept_nl_cost", False),
        )
        summary.append(rec)
...
def main() -> None:
    ...
    r.add_argument("--target", choices=["local", "sf", "neolink"], required=True)
    ...
    r.add_argument("--i-accept-nl-cost", action="store_true")
