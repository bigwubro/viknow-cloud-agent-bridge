...
LMCache 已从现网撤掉（hybrid 不匹配）。跨 P 共享 prefix 的试验：`./run-pd-3p1d-mooncake.sh`（Nixl 仍管本请求 P→D，Mooncake Store 只挂在三路 P，TCP、无 RDMA）。出词侧不能挂 MooncakeStoreConnector：0.29 在 consumer 上 `build_connector_meta` 会断言 `Missing current block table`。回退 Nixl-only：`./run-pd-3p1d.sh`。三路 P 默认按在途最少分载。
