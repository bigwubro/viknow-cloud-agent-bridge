...
## P 只做预填充：token 预算调到能站住的最高并发（2026-09-19 05:33 UTC）

P 现为 `--max-num-batched-tokens 65536`、`--max-num-seqs 256`、`--kv-cache-memory 21474836480`（20 GiB）。出词仍 16384 / 0.75。一张 P 一个 step 大约能塞 **3 条** 2 万 token 的尾巴（原先 32768 大约 1.6 条）。

试过但起不来：131072（激活把单进程顶到 62 GiB，建 KV OOM）；0.90 / 0.84（四路互见后空闲约 78 GiB，要 85 / 80 GiB）；65536 + 默认 KV（GPU0 能起，GPU1/2 被 `:8502` embed / `:8503` whisper 各约 16 GiB 挤爆）；`expandable_segments:True`（NIXL/Mooncake 拒）。没有动 8501–8506 上的进程。

同一套 B，c32 × 90s（`/tmp/pd-029-p65k-c32`）：

| 项 | 32768 · c32 × 10 分钟 | **65536 + 20GiB KV · c32 × 90s** |
|---|---|---|
| 成功/提交 | 1219/1219 | **280/280** |
| 每秒完成 | 1.971 | **2.727** |
| 中位 / 95 分位 | 13.52s / 33.38s | **8.37s / 31.00s** |
| P 排队 / 预填充 | 8.61s / 5.84s | **4.90s / 4.27s** |
| 抢占 | 720 / 1220 | **55 / 281** |
| picks | +409/+415/+396 | **+90/+91/+100** |

NIXL 19ms，D 出词 1.13s，MTP 21526/30068 = 71.6%。90s 窗口更接近原先 10 分钟档的前 2 分钟（当时中位 8.71s、约 3 QPS），不是整段 10 分钟的稳态。短请求 `1+1=2`。`:8500` 200。其他容器未动。

## 约 2 万 token · 32 并发：自建 / 硅基 / Neolink（2026-09-19 05:59–06:15 UTC）

同一套 B，目标 prompt 20000、独特尾巴 9000，本地校准 shared 11057 + unique 8866，探活 **19934**。thinking 关，`max_tokens=184`，每档 180s。现网未改（3P+1D，P 65536 / 20 GiB KV）。HTML：`report-b-c32-20k.html`。详表：`RESULTS_B20K.md`。

| | 成功/提交 | 每秒完成 | 中位 / 95 / 最大 | 单条成本 |
|---|---|---|---|---|
| 自建 `:8500` | **698/698** | **3.702** | **6.30s / 20.41s / 50.85s** | 卡费 **¥0.00171** |
| 硅基 `Qwen/Qwen3.6-35B-A3B` | 91/155 | 0.435 | 33.30s / 42.50s / 42.79s | 实付 **¥0.0412**（本档 ¥3.75） |
| Neolink `qwen3.6-flash` | 1470/2597 | **7.908** | **3.32s / 6.78s / 11.09s** | 百炼参考 ¥0.0276，不是发票 |

硅基失败 64 全是 429。Neolink 失败 1127 全是 500。自建 0 失败。`:8500` 200。其他容器未动。
