...
- 对象：ACK `viknow2-online-prod`、236 `viknow-ai-alb-proxy`、RDS/Tair/OSS/ACR（只读）；产出 `docs/aliyun-architecture.html`
- 动作：核对 `deploy/ack/*.yaml`、`config/online/infrastructure-inventory.md`、236 Nginx 反代配置；公网探测 Web/Langfuse/LLM 入口。**未创建/修改/删除云资源，未登录控制台，无 kubeconfig 故未 `kubectl get`。**
- 结果：成功（图已画出；ACK API 仍不可达）
- 证据：
  - `http://ai.qingxiang.tech/` → 200，`Server: nginx/1.29.4`（236 反代 → `115.29.178.3:15174` → NodePort 30542）
  - `http://viknow.ai.qingxiang.tech:8088/` → 200
  - `http://langfuse.ai.qingxiang.tech/api/public/health` → 200 `{"status":"OK","version":"3.163.0"}`（ALB HTTP，相对 8 月底 inventory「待备案」已通）
  - `https://…` :443 失败；`https://ai.qingxiang.tech:8443/v1/models` → 401（网关在线）
- 残留：无登录态；`viknow-env.env.example` 仍写废弃 Tair `tt-bp1logor6p50p15b1`（未改业务仓）
